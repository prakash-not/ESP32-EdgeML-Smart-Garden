...
1.  **CRITICAL SAFETY:** Is the Water Tank empty (`< 1000`)? → *Lock Pump & Alert.*
2.  **SOIL HEALTH:** Is moisture `> 3800` (Very Dry)? → *Water.* Is it `< 1600` (Wet)? → *Stop.*
3.  **AIR QUALITY:** Is Gas `> 2400`? → *Trigger "Root Rot" Danger Alert.*
4.  **ENVIRONMENT:** Is it Night time? → *Turn off Lights.*
...
## 📸 Hardware Prototype
Here is the physical implementation of the system:

![Hardware Setup](diagrams/hardware_setup.jpg)
